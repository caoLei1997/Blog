// 文章数据与查询函数（Server Component 与 Route Handler 共用，避免 self-fetch）
import { fakeMarkdown } from '@/app/api/articles/[id]/mock.js';
...
export async function getArticleById(id: string): Promise<ArticleDetail | null> {
  return articleDetails.find((p) => p.id === id) ?? null;
}
